      ...
      <div style={{...styles.items, textAlign: 'left'}}>
        {theItems && theItems.map((c, idx) => (
          <div key={'itemShow' + idx}>
            ...
            <span><img onClick={() => this.isEdit(idx)} key={'content-edit' + idx} style={styles.editIcon} alt="cost of item" src={require('../../assets/img/item-edit.png')}/></span>
            <span onClick={this.itemDelete.bind(this, c.timestamp)}><img key={'content-delete' + idx} style={styles.deleteIcon} alt="cost of item" src={require('../../assets/img/item-delete.png')}/></span>
          </li>}
            {isEdit[idx] && <div key={'category' + idx}  style={{display: 'flex', alignItems: 'center', justifyContent: 'flex-start', width: '100%'}}>
              <li style={{...styles.item,listStyleType: 'none'}} key={'editItem' + idx}>
                ...
                <input type="text" style={styles.inputFrame} value={editValue[idx]}  ref={(value) => { this.value = value; }}
                       onChange={(c) => this.editValue(c.target.value, idx)}/>
                <label style={styles.editTitle}>備註: </label>
                <input type="text" style={styles.inputFrame} value={editContent[idx]} ref={(content) => { this.content = content; }}
                       onChange={(c) => this.editContent(c.target.value, idx)}/>
                <button type="submit" id="item-submit" key={'edit-item-submit' + idx}style={{...styles.editFinish}} onClick={() => this.editFinish(c.timestamp,idx)}>完成
                </button>
              </li>
            </div>}
          </div>
        ))}
        {/*{isEdit && theItems && theItems.map((c, idx) => (*/}
        {/*  <div key={'category' + idx}  style={{display: 'flex', alignItems: 'center', justifyContent: 'flex-start', width: '100%'}}>*/}
        {/*      <li style={{...styles.item,listStyleType: 'none'}} key={'item' + idx}>*/}
        ...
        {/*        <input type="text" style={styles.inputFrame} value={c.itemValue}*/}
        {/*               onChange={(c) => this.inputValue(c.target.value)}/>*/}
        {/*        <label style={styles.inputTitle}>備註: </label>*/}
        {/*        <input type="text" style={styles.inputFrame} value={c.itemContent}*/}
        {/*               onChange={(c) => this.inputContent(c.target.value)}/>*/}
        {/*        <button type="submit" id="item-submit" style={{...styles.editFinish}} onClick={() => this.editFinish()}>完成*/}
        {/*        </button>*/}
        {/*      </li>*/}
        {/*  </div>*/}
        {/*))}*/}
